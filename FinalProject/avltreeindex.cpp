...
Term*& AVLTreeIndex::find (string word){
    ...
    if(root == NULL){
        ...
        currWord = root->data->get_name();
    }
    if(root->left == NULL && root->right ==NULL){
        return nll;
    }
    if ((word < currWord) && (root->left)){
        foundNode = foundNode->left;
        return continue_search(foundNode, word);
    }else if((word > currWord) && (root->right)){
...
Term*& AVLTreeIndex::continue_search(AVL_Node*& currNode, string word){
    string word2 = currNode->data->get_name();
    Term* nll = NULL;
    if(currNode->left == NULL && currNode->right == NULL){
        return nll;
    }
    if ((word < word2) && (currNode->left)){
        currNode = currNode->left;
        return continue_search(currNode, word);
    }else if((word > word2) && (currNode->right)){
